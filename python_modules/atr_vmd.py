    ...
    def timing_summary(self, reset=False):
        ...
        if self._kind == "detector":
            ...
            for key in (
                ...
                if key in summary:
                    out[key] = summary[key]
            if "total_ms_avg" in summary and "registration_ms_avg" in summary:
                det = summary["total_ms_avg"] - summary["registration_ms_avg"]
                out["detection_sum_ms_avg"] = max(0.0, det)
            return out
        return summary

try:
    from ._register_detect_cpp import RegisterAndDetectTranslationGPUCpp as _RegisterAndDetectTranslationGPUCpp
    _REGISTER_DETECT_CPP_IMPORT_ERROR = None
except Exception as exc:
    _RegisterAndDetectTranslationGPUCpp = None
    _REGISTER_DETECT_CPP_IMPORT_ERROR = str(exc)
try:
    from ._register_detect_cpp import RegisterAndDetectHomographyGPUCpp as _RegisterAndDetectHomographyGPUCpp
except Exception:
    _RegisterAndDetectHomographyGPUCpp = None
try:
...
class ATR_VMD:
    def __init__(self, reference, config_loader):
        ...
        min_contour_area = int(cfg.get("detection.min_contour_area", 50))
        max_contour_area = int(cfg.get("detection.max_contour_area", 1500))
        border_gate_percent = float(cfg.get("detection.border_gate_percent", 1.0))
        ...
        self.detector = Detector(
            ...
            border_gate_percent=border_gate_percent,
            use_cuda=use_cuda,
            use_cpp=use_cpp,
        )
        ...
        if (use_cpp and use_cuda
            ...
            and _RegisterAndDetectTranslationGPUCpp is not None):
            try:
                from . import video_reader
                input_is_gray = bool(video_reader.input_is_gray())
                if _RegisterAndDetectTranslationGPUCpp.is_available():
                    try:
                        self._register_and_detect = _RegisterAndDetectTranslationGPUCpp(
                            reference,
                            input_is_gray,
                            downscale_factor=downscale_factor,
                            detect_scale=detect_scale,
                            ...
                            phase_use_cached_fft=phase_use_cached_fft,
                            enable_timing=enable_timing,
                            return_registered=bool(draw_output),
                        )
                    except TypeError:
                        self._register_and_detect = _RegisterAndDetectTranslationGPUCpp(
                            reference,
        ...
        if (use_cpp and use_cuda
            and registration_mode == "homography"
            and _RegisterAndDetectHomographyGPUCpp is not None):
            try:
                from . import video_reader
                input_is_gray = bool(video_reader.input_is_gray())
                feature_name = getattr(feature_extractor, "name", str(feature_extractor))
                matcher_name = getattr(matcher, "name", str(matcher))
                if _RegisterAndDetectHomographyGPUCpp.is_available():
                    try:
                        self._register_and_detect = _RegisterAndDetectHomographyGPUCpp(
                            reference,
                            input_is_gray,
                            downscale_factor=downscale_factor,
                            detect_scale=detect_scale,
                            ...
                            ransac_reproj_threshold=ransac_reproj_threshold,
                            min_inliers=min_inliers,
                            reanchor_boost_frames=0,
                            reanchor_boost_lr=0.0,
                            enable_timing=enable_timing,
                            return_registered=bool(draw_output),
                        )
                    except TypeError:
                ...
                if use_cpp:
                    raise
                print("ATR_VMD: C++ homography register+detect init failed, using Python pipeline")
        if use_cpp and use_cuda and registration_mode in ("translation", "homography") and self._register_and_detect is None:
            reason = "unknown"
            if _REGISTER_DETECT_CPP_IMPORT_ERROR is not None:
                reason = _REGISTER_DETECT_CPP_IMPORT_ERROR
            elif registration_mode == "translation" and _RegisterAndDetectTranslationGPUCpp is not None:
                reason = f"translation.is_available={_RegisterAndDetectTranslationGPUCpp.is_available()}"
            elif registration_mode == "homography" and _RegisterAndDetectHomographyGPUCpp is not None:
                reason = f"homography.is_available={_RegisterAndDetectHomographyGPUCpp.is_available()}"
            raise RuntimeError(
                "ATR_VMD: C++ register+detect pipeline requested but unavailable"
                f" (reason: {reason})."
            )
        ...
        kalman_area_gate_min_scale = cfg.get(
            "tracker.kalman.area_gate_min_scale__for_match",
            cfg.get("tracker.kalman.area_gate_min_scale", 0.5),
        )
        kalman_area_gate_max_scale = cfg.get(
            "tracker.kalman.area_gate_max_scale__for_match",
            cfg.get("tracker.kalman.area_gate_max_scale", 2.0),
        )
        ...
        if use_cpp and _KalmanIoUTrackerCpp is not None:
            try:
                self.tracker = _KalmanIoUTrackerCpp(
                    ...
                    kalman_area_gate_min_scale,
                    kalman_area_gate_max_scale,
                )
            except TypeError:
                # Backward-compatibility with older pybind builds that expose only 6 args.
                self.tracker = _KalmanIoUTrackerCpp(
                    kalman_iou_thresh,
                    int(kalman_max_lost),
                    kalman_min_move,
            ...
            self.tracker = KalmanIoUTracker(
                ...
                area_gate_min_scale=kalman_area_gate_min_scale,
                area_gate_max_scale=kalman_area_gate_max_scale,
            )
    ...
    def _print_pipeline_summary(self, registration_mode, use_cuda):
        if self._register_and_detect is not None:
            print("")
            print("Pipeline:")
            print(f"  register+detect: {registration_mode} | cpp | gpu")
    ...
    def register_and_detect(self, frame):
        if self._register_and_detect is not None:
            ...
            if ref_events is not None and len(ref_events):
                current_reg_frame = self.frame_count

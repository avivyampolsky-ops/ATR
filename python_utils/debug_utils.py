...
class DataStreamer:
    ...
    def _init_source(self):
        ...
        if os.path.isdir(self.data_path):
            ...
            from python_modules import video_reader
...
class DebugUtils:
    def __init__(self, config, data_path, out_dir, stream_fps, stream_is_gray):
        ...
        self.incrimination_thresh = int(config.get("tracker.kalman.incrimination_thresh", -1))
        ...
        self._writer = None
        self._handshake_log = None
        self._stream_fps = stream_fps
        self._stream_is_gray = stream_is_gray
        self._homography_log = None
        self.loop_calls = 0
        if self.debug_mode and self.incrimination_thresh >= 0:
            handshake_path = os.path.join(self.out_dir, "vmd_handshake.csv")
            self._handshake_log = open(handshake_path, "w", encoding="ascii")
            self._handshake_log.write("frame_input,frame_proc,track_id,age,x,y,w,h\n")
            self._handshake_log.flush()
    ...
    def close(self):
        ...
        self.close_handshake_log()

    def close_handshake_log(self):
        # Close the incrimination handshake log if it was created.
        if self._handshake_log is not None:
            self._handshake_log.close()
            self._handshake_log = None
    ...
    def draw_tracks(self, frame, tracks):
        ...
        if not self.debug_mode or frame is None or tracks is None:
            return frame
        out = frame.copy()
        default_color = 255 if out.ndim == 2 else (0, 255, 0)
        incriminated_color = 255 if out.ndim == 2 else (0, 0, 255)
        label_color = 255 if out.ndim == 2 else (0, 0, 255)
        for track_id, entity in tracks.items():
            if not entity.moving:
                continue
            if self.incrimination_thresh >= 0:
                green_age = (3 * self.incrimination_thresh + 3) // 4
                if entity.age < green_age:
                    continue
                color = incriminated_color if entity.age >= self.incrimination_thresh else default_color
            else:
                if entity.age < self.track_min_age:
                    continue
                color = default_color
            x, y, w, h = entity.bbox
            ...
            cv2.putText(
                out,
                f"ID {track_id} AGE:{int(entity.age)}",
                (x, y - 6),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                label_color,
    ...
    def _append_registrator_summary(self, lines, summary):
        ...
        reg_sum = None
        if "registration_sum_ms_avg" in summary:
            reg_sum = summary["registration_sum_ms_avg"]
        elif "registration_ms_avg" in summary:
            reg_sum = summary["registration_ms_avg"]
        elif "registration" in summary:
            reg_sum = summary["registration"]
        if reg_sum is not None:
            lines.append(f"  registration sum: {reg_sum:.3f}")
    ...
    def _append_detector_summary(self, lines, summary):
        ...
        det_sum = None
        if "detection_sum_ms_avg" in summary:
            det_sum = summary["detection_sum_ms_avg"]
        elif "total_ms_avg" in summary:
            det_sum = summary["total_ms_avg"]
        if det_sum is not None:
            lines.append(f"  detection sum: {det_sum:.3f}")
    ...
    def process_image_outputs(self, tracker, frame_idx, ref_events, homography_debug):
        tracks = tracker.tracker.get_tracks()
        self.write_handshake(frame_idx, tracker, tracks)
        if not self.debug_mode or not self.draw_output:
            return
        current_frame_idx = max(0, tracker.frame_count - 1)
    ...
    def _get_registration_margin(self, tracker):
        registrator = getattr(tracker, "registrator", None)
        if registrator is None:
            return 0
        margin = getattr(registrator, "margin", None)
        if margin is not None:
            return int(margin)
        impl = getattr(registrator, "_impl", None)
        if impl is not None:
            margin = getattr(impl, "margin", None)
            if margin is not None:
                return int(margin)
        return 0

    def _to_self_frame_bbox(self, tracker, entity):
        x, y, w, h = entity.bbox
        x = int(round(x))
        y = int(round(y))
        w = int(round(w))
        h = int(round(h))
        if self.registration_mode != "translation":
            return x, y, w, h
        margin = self._get_registration_margin(tracker)
        dx, dy = tracker.get_last_shift()
        return (
            int(round(x + margin - float(dx))),
            int(round(y + margin - float(dy))),
            w,
            h,
        )

    def write_handshake(self, frame_idx, tracker, tracks):
        # Write and flush detections whose age reached incrimination threshold.
        if self._handshake_log is None or tracks is None or self.incrimination_thresh < 0:
            return
        frame_proc = max(0, int(getattr(tracker, "frame_count", 1)) - 1)
        for _, entity in tracks.items():
            if int(entity.age) != self.incrimination_thresh:
                continue
            x, y, w, h = self._to_self_frame_bbox(tracker, entity)
            track_id = int(getattr(entity, "id", -1))
            age = int(entity.age)
            self._handshake_log.write(
                f"{frame_idx},{frame_proc},{track_id},{age},{x},{y},{w},{h}\n"
            )
            self._handshake_log.flush()

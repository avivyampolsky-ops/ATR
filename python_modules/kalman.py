...
class KalmanIoUTracker:
    def __init__(
        ...
        dist_gate_scale=2.0,
        easy_iou_thresh=0.6,
        area_gate_min_scale=0.5,
        area_gate_max_scale=2.0,
    ):
        ...
        self.area_gate_min_scale = area_gate_min_scale
        self.area_gate_max_scale = area_gate_max_scale
        if self.area_gate_min_scale <= 0.0:
            raise ValueError("area_gate_min_scale must be > 0.0")
        if self.area_gate_max_scale < self.area_gate_min_scale:
            raise ValueError("area_gate_max_scale must be >= area_gate_min_scale")
    ...
    def _build_iou_matrix(self, track_items, observations: np.ndarray) -> Optional[np.ndarray]:
        ...
        for t_idx, (_, entity) in enumerate(track_items):
            ...
            # ---------- EMA AREA GATE ----------
            area_mask = (
                (obs_area >= self.area_gate_min_scale * ema_area)
                & (obs_area <= self.area_gate_max_scale * ema_area)
            )
            # ---------- DISTANCE GATE ----------

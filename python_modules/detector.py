...
class BackgroundSubtractorMOG2:
    def __init__(
        ...
        self._border_gate_percent = max(0.0, min(49.0, float(border_gate_percent)))
        # Mask cleanup for bolder blobs and less speckle noise.
        self._min_contour_area = int(min_contour_area)
        self._max_contour_area = int(max_contour_area)
    ...
    def _touches_border(self, bbox):
        ...
        if self._border_gate_percent <= 0.0:
            return False
        margin_x = int(round(w_full * (self._border_gate_percent / 100.0)))
        margin_y = int(round(h_full * (self._border_gate_percent / 100.0)))
        return (
            x <= margin_x
            or y <= margin_y
            or (x + w) >= (w_full - margin_x)
            or (y + h) >= (h_full - margin_y)
        )

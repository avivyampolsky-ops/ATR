...
private:
    ...
    bool touches_border(const cv::Rect &bbox) const {
        ...
        int margin_x = static_cast<int>(std::round(full_w_ * (border_gate_percent_ / 100.0f)));
        int margin_y = static_cast<int>(std::round(full_h_ * (border_gate_percent_ / 100.0f)));
        return (
            bbox.x <= margin_x ||
            bbox.y <= margin_y ||
            (bbox.x + bbox.width) >= (full_w_ - margin_x) ||
            (bbox.y + bbox.height) >= (full_h_ - margin_y)
        );
    }
};
...
PYBIND11_MODULE(_detector_cpp, m) {
    ...
    py::class_<BackgroundSubtractorMOG2GPUCpp>(m, "BackgroundSubtractorMOG2GPUCpp")
        .def(py::init<const py::tuple &, bool, bool, float, float, int, float, int, int, float>(),
             py::arg("frame_shape"),
